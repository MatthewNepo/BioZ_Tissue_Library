function f = ExVivoDataPlot_opt_archive(substance_name,Substance,Substance_sampleavgs,Substance_sampleavgs_avg,freq,sample_num)
    ...
    if rem(sample_num,2) ~= 0
        Fig_x = (sample_num+1)/2;
        Fig_y = 2;
        ...
        Title = [substance_name ': Sample Averages'];
        title(Title)
        legend(legendCell,'Average of Samples')
        x0 = 150; y0 = 35; width = 1000; height = 600;
        set(gcf,'position',[x0,y0,width,height])
        f.Units = 'normalized';
        lineWidth = 5; % Whatever you want.
